...
def test_statement_application():
    ...
    R = ElementrySet(quantor=Quantor.DEFINE, association='R')
    ...
    y = Variable(quantor=Quantor.FORALL, binding_quantity=(Y, ), association='y')
    Bx = Function(quantor=Quantor.DEFINE, binding_quantity=(Set(quantor=Quantor.FORALL, binding_quantity=(FunctionSet(quantor=Quantor.FORALL, binding_quantity=(X, X)), R, X)), PowerSet(quantor=Quantor.FORALL, binding_quantity=(X, ))), association='B_{dx}')
    By = Function(quantor=Quantor.DEFINE, binding_quantity=(Set(quantor=Quantor.FORALL, binding_quantity=(FunctionSet(quantor=Quantor.FORALL, binding_quantity=(Y, Y)), R, Y)), PowerSet(quantor=Quantor.FORALL, binding_quantity=(Y, ))), association='B_{dy}')
    Bz = Function(quantor=Quantor.DEFINE, binding_quantity=(Set(quantor=Quantor.FORALL, binding_quantity=(FunctionSet(quantor=Quantor.FORALL, binding_quantity=(Z, Z)), R, Z)), PowerSet(quantor=Quantor.FORALL, binding_quantity=(Z, ))), association='B_{dz}')
    epsilon = Variable(quantor=Quantor.FORALL, binding_quantity=(R, ))
    delta = Variable(quantor=Quantor.EXISTS, binding_quantity=(R, ))

    g_continuity_left = Node(g, child_nodes=[[(Node(By, child_nodes=[[(Node(dy), 1)], [(Node(delta), 1)], [(Node(y), 1)]]), 1)]])     # g(B_{delta}^{dy}(y))
    g_continuity_right = Node(Bz, child_nodes=[[(Node(dz), 1)], [(Node(epsilon), 1)], [(Node(g, child_nodes=[[(Node(y), 1)]]), 1)]])   # B_{epsilon}^{dz}(g(y))
    gf_continuity_right = Node(Bz, child_nodes=[[(Node(dz), 1)], [(Node(epsilon), 1)], [(Node(g, child_nodes=[[(Node(f, child_nodes=[[(Node(x), 1)]]), 1)]]), 1)]])  # B_{epsilon}^{dz}(g(f(x)))
    ...
    assert applied_g_term == Node(g, child_nodes=[[(Node(By, child_nodes=[[(Node(dy), 1)], [(Node(delta), 1)], [(Node(f, child_nodes=[[(Node(x), 1)]]), 1)]]), 1)]])

    f_continuity_right = Node(f, child_nodes=[[(Node(Bx, child_nodes=[[(Node(dx), 1)], [(Node(delta), 1)], [(Node(x), 1)]]), 1)]])  # f(B_{delta}^{dx}(x))
    f_continuity_left = Node(By, child_nodes=[[(Node(dy), 1)], [(Node(epsilon), 1)], [(Node(f, child_nodes=[[(Node(x), 1)]]), 1)]])  # B_{epsilon}^{dy}(f(x))
    ...
    assert applied_gf_term == Node(g, child_nodes=[[(Node(f, child_nodes=[[(Node(Bx, child_nodes=[[(Node(dx), 1)], [(Node(delta), 1)], [(Node(x), 1)]]), 1)]]), 1)]])
