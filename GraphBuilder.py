...
class _ArgumentSlotBuilder:
    ...
    edge_sequence: Tuple[_EdgeBuilder, ...]
...
class GraphBuilder:
    ...
    def add_edge_to_slot(node: _NodeBuilder, edge: _EdgeBuilder, idx: int):
        ...
        node.argument_slots[idx].append(edge)
    ...
    def build(self):
        ...
        for builder_node, node in final_nodes.items():
            for builder_slot, slot in zip(builder_node.argument_slots, builder_node.argument_slots):
        ...
        if not self._root_node:
            # TODO: Implement search to return node that is never pointed to by any edge; infinite circles
