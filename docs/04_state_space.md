...
## 3. Implementierung
Wir definierenen einen neue Graphen-Klasse, die den Graphen nicht vorgenneriert, sondern rekursiv erstellt, wir wenden einfach die alle gegebenen Aussagen auf jeden momentariness untersuchten front-Zustand an und speichern die nächsten. Für die Zurückführbarkeit der Beweise nutzen wir als States eine neue Beweis-Klasse, die sowohl den aktuellen ExpressionTree, sowie die vorigen Transformationen speichert. Falls wir von $T_S$ starten und $T_E$ erreichen, hat das Programm einen Beweis gefunden, anders herrum funkrioniert das auf selbe Weise (diese lässt später auch bidirektionale Zustandsraum suchen zu). Eine Zustandsraumsuche erfolgt mit einem Algorithmus (wie BFS, A* oder Greedy).
